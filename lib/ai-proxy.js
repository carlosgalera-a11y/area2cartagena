...
(function (global) {
  ...
  var AIProxy = {
    ...
    /**
     * Envia un prompt + imagen al proxy de vision.
     * @param {Object} opts
     * @param {string} opts.prompt              Obligatorio, <=4000 chars.
     * @param {string} [opts.systemPrompt]      Instruccion de sistema.
     * @param {string} [opts.imageDataUrl]      "data:image/jpeg;base64,...."
     * @param {string} [opts.imageBase64]       alternativa: base64 pelado
     * @param {string} [opts.imageMimeType]     obligatorio si no se pasa dataUrl
     * @param {number} [opts.maxTokens]         1..4096
     * @param {number} [opts.temperature]       0..2
     * @param {number} [opts.timeoutMs]         default 90000 (imagenes tardan)
     */
    vision: function (opts) {
      opts = opts || {};
      if (!opts.prompt) return Promise.reject(new Error('prompt_required'));
      if (!opts.imageDataUrl && !opts.imageBase64) {
        return Promise.reject(new Error('image_required'));
      }
      var payload = {
        prompt: String(opts.prompt),
        systemPrompt: opts.systemPrompt || '',
      ...
      if (opts.imageDataUrl) payload.imageDataUrl = opts.imageDataUrl;
      else {
        payload.imageBase64 = opts.imageBase64;
        payload.imageMimeType = opts.imageMimeType || 'image/jpeg';
      }
      return callApi('/api/ai/vision', {
        body: payload,
