...
// CONFIG legacy — provider ignorado ya (askAi enruta todo).
var CONFIG={provider:"askai",groqKey:"",groqModel:"",qwenKey:"",qwenModel:""};
try{var s=localStorage.getItem("notebook_ai_cfg_v3");if(s)Object.assign(CONFIG,JSON.parse(s));}catch(e){}
// ENDPOINTS legacy vaciado — todas las llamadas IA pasan por window.askAi.
var ENDPOINTS={};
function ep(){return {url:'',getKey:function(){return ''},getModel:function(){return ''},prefix:''};}
function isReady(){return true;}
function updateStatus(){var el=document.getElementById("statusBadge"),b=document.getElementById("modelBadge"),i=document.getElementById("modelInfo");if(!el)return;el.className="nav-status ok";el.textContent="✅ IA Conectada";b.textContent="askAi";i.textContent="Cloud Function · europe-west1";}
function cambiarProvider(){/* no-op: provider único vía askAi */}
...
async function llamarIA(up,sp){
  ...
  /* ─── Medida 4: rate limiting cliente (además del que aplica la Cloud Function) ─── */
  try { aiRateLimiter.check(); } catch(e) { return e.message; }
  /* ═══ Todas las llamadas IA pasan por la Cloud Function askAi (europe-west1) ═══ */
  try{
    if(typeof window.askAi!=='function'){
      return '⚠️ Cliente IA no cargado. Recarga la página.';
    }
    var res = await window.askAi({
      type: 'educational',
      prompt: up,
      systemPrompt: sp || 'Eres un asistente médico. Responde en español.',
    });
    var c = (res && res.text) || '';
    c = c.replace(/<think>[\s\S]*?<\/think>/g,'').trim();
    if(!c) return '⚠️ Respuesta vacía de la IA.';
    /* EU AI Act Art. 52 — transparencia del modelo */
    lastAIModel = (res.provider||'?')+' · '+(res.model||'?');
    /* Historial local con offuscación suave para el dashboard usuario.
       Auditoría formal la hace la Cloud Function en users/{uid}/aiRequests. */
    try{
      secureStore.set('aiHistory', JSON.stringify((function(){
        var h=JSON.parse(secureStore.get('aiHistory')||'[]');
        h.push({q:up.substring(0,100),s:currentCategory||'',t:Date.now(),m:lastAIModel});
...
function openDocModal(idx){
  ...
  if(d.body){
    document.getElementById('docModalContent').innerHTML=(d.description?'<p style="color:var(--text-muted,#555);font-style:italic;margin-bottom:14px;padding-bottom:10px;border-bottom:1px solid var(--border,#e2e8f0);">'+d.description+'</p>':'')+d.body;
  } else {
...
function actualizarUI(){var docs=documents[currentCategory]||[];document.getElementById("documentosList").innerHTML=docs.length===0?'<div class="empty-state"><div class="empty-state-icon">📄</div><p>No hay documentos</p></div>':docs.map(function(d,i){var isExternal=d.url&&(d.url.endsWith('.pdf')||d.url.startsWith('http'))&&d.url.indexOf('protocolos-nuevas-especialidades')<0;var btn=isExternal?'<a href="'+d.url+'" target="_blank" class="doc-btn">Abrir</a>':'<button onclick="openDocModal('+i+')" class="doc-btn" style="border:none;cursor:pointer;">Ver</button>';var meta=d.body?'✅ contenido · '+(d.type||'protocolo'):((d.size_mb||0)+' MB · '+(d.type||'protocolo'));return'<div class="doc-item"><div><div class="doc-name">'+d.name+'</div><div class="doc-size">'+meta+'</div></div>'+btn+'</div>'}).join("");
...
// Legacy: nombres conservados para callers aún no migrados. askAi gestiona todo.
var SCAN_GROQ_KEY_DEFAULT="";var EMBEDDED_GROQ_KEY="";
var SCAN_GROQ_MODEL_DEFAULT="";
function getScanGroqKey(){return '';}
function getScanGroqModel(){return '';}
...
// Stubs legacy. Nunca se vuelve a guardar ni cargar clave Groq en Firestore.
// El doc `config/groq_api_key` existente debe borrarse manualmente (ver audit).
function saveGroqKeyToFirestore(){ /* no-op */ }
function loadGroqKeyFromFirestore(){ /* no-op */ }
var SCAN_GROQ_KEY='';
var SCAN_GROQ_MODEL='';
var SCAN_PROMPTS={
...
torax:"Eres radiólogo torácico docente. Lectura sistemática + detección de nódulos.\n\nREFERENCIAS (modelos y datasets que integras):\n• Modelos: TorchXRayVision (mlmed/torchxrayvision), CheXNet (DenseNet-121), CheXpert-Chen, Lunit INSIGHT CXR, Siemens AI-Rad Companion, Google CXR Foundation.\n• Datasets: CheXpert (Stanford, 224K, 14 etiq), MIMIC-CXR (MIT, 377K + informes), NIH ChestX-ray14 (112K), PadChest (UAlicante, 160K jerárquico), VinDr-CXR (Vietnam, 18K bbox), VinDr-PCXR (pediátrico), Open-i (Indiana, 7K pareado Rx+informe), RSNA Pneumonia (Kaggle 30K bbox), SIIM-ACR Pneumothorax (12K máscaras), CANDID-PTX, JSRT (247 nódulos anotados), LIDC-IDRI + LUNA16 + NODE21 (nódulos), TBX11K + Shenzhen + Montgomery (TB), COVIDx-CXR-3 + BIMCV-COVID19 + MIDRC-RICORD + COVID-19 Image Data Collection (Cohen), BRAX (Brasil 40K), ChestX-Det10 (10 clases bbox), Object-CXR (cuerpos extraños), CheXlocalize, CANDID-II.\n\nMÉTODO: 12 zonas (3 niveles × 2 regiones × 2 lados). Revisa zonas ciegas: ápices, hilios, retrocardíaca, senos costofrénicos, detrás de costillas/diafragma.\n\nINFORMA EN ESTE ORDEN:\n\n1. CALIDAD TÉCNICA: proyección (PA/AP), rotación, inspiración (>6 arcos ant.), penetración.\n\n2. NÓDULOS/MASAS (PRIORITARIO). Cada lesión focal: localización (lado/zona/central-periférica-subpleural), tamaño (vértebra≈25mm), forma (redonda/oval/lobulada/irregular/espiculada), densidad (sólido/subsólido/vidrio deslustrado), bordes (lisos/mal def/espiculados/corona radiata), calcificación (central/laminada/popcorn/excéntrica sospechosa), cavitación (pared fina<4mm vs gruesa>15mm), broncograma aéreo, relación cisuras/vasos/pleura. Clasif.: micronódulo<3mm · 3-6 · 6-8 · 8-30 · masa>30.\n\n3. MEDIASTINO: ICT(<0,5), hilios, tráquea, botón aórtico, ventana AP, líneas paratraqueal/paraespinal.\n\n4. CAMPOS PULMONARES: transparencia, intersticial/alveolar, consolidación, atelectasia, hiperinsuflación.\n\n5. PLEURA: derrame (menisco, borrado seno), neumotórax (línea pleural, surco profundo), placas.\n\n6. ÓSEO: lesiones líticas/blásticas, fracturas.\n\n7. DIAFRAGMA/ABDOMEN SUP.: aire subdiafragmático, cámara gástrica.\n\n8. DISPOSITIVOS: CVC, marcapasos, TET, drenajes.\n\n9. PROBABILIDADES CheXpert-14 (0-100%, NEGRITA si >20%): Atelectasia, Cardiomegalia, Consolidación, Edema, Derrame, Enfisema, Fibrosis, Hernia, Infiltración, Masa, Nódulo, Engrosamiento pleural, Neumonía, Neumotórax.\n\n10. IMPRESIÓN: hallazgo principal + sospecha (baja/intermedia/alta), DDx ordenado, riesgo de malignidad si nódulo (Brock Model).\n\n11. RECOMENDACIÓN: Fleischner 2017 (nódulo incidental: <6mm bajo riesgo→no seguir; 6-8mm→TC 6-12m; >8mm→TC/PET-TC/biopsia), Lung-RADS cribado, TCAR/PET-TC/broncoscopia/biopsia según sospecha.\n\n⚠️ DOCENTE. No sustituye lectura radiológica. Hallazgo sospechoso→TC alta resolución + Radiología.",
...
var SCAN_MODELS={
    ...
    torax:{model:"TorchXRayVision + CheXNet + Google CXR Foundation",dataset:"CheXpert · MIMIC-CXR · NIH ChestX-ray14 · PadChest · VinDr-CXR · RSNA Pneumonia · SIIM-ACR · JSRT · LIDC-IDRI · LUNA16 · NODE21 · TBX11K · COVIDx-CXR-3 · BRAX · Open-i · ChestX-Det10 · Object-CXR",arch:"DenseNet-121, ResNet, ViT preentrenados multi-dataset (24+ bases públicas)",commercial:"Lunit INSIGHT CXR, Siemens AI-Rad Companion, Aidoc, Qure.ai qXR",repo:"GitHub: mlmed/torchxrayvision (librería estándar) + google-health/cxr-foundation",precision:"Detecta 18+ patologías: neumonía, cardiomegalia, derrame, neumotórax, nódulos, TB, COVID, enfisema, fibrosis, etc.",formato:"Python pip install torchxrayvision → imagen en escala de grises → probabilidades + mapa de calor Grad-CAM",nota:"Ensemble sobre 24 datasets públicos (>1.1 M Rx tórax). Benchmark: CheXpert AUROC>0.92 para 14 patologías."},
...
async function scanAnalyze(){
    ...
    // Ficha post-análisis (AI Act art. 13 transparencia al usuario)
    var _sm = SCAN_MODELS[scanType] || {};
    var _region = (usedModel||'').toLowerCase().indexOf('gemini')>=0 ? 'europe-west1 (UE)' : (usedModel||'').toLowerCase().indexOf('deepseek')>=0 ? 'CN · fallback UE vía OpenRouter' : (usedModel||'').toLowerCase().indexOf('qwen')>=0 ? 'Intl · routing variable' : 'UE';
    var _benchmarkShort = (_sm.precision||'').substring(0,110);
    var transparencyHtml = '<div style="background:#f8fafc;border:1px solid #cbd5e1;border-radius:6px;padding:10px 12px;margin-top:10px;font-size:.74rem;line-height:1.5;color:#475569"><strong style="color:#0f172a">ℹ️ Transparencia (EU AI Act art. 13):</strong> Modelo <code style="background:#e2e8f0;padding:1px 5px;border-radius:3px;font-family:monospace">'+(usedModel||'—')+'</code> · Región procesamiento: '+_region+' · Benchmark publicado: '+(_benchmarkShort||'no declarado')+' · <a href="/docs/legal/scan-ai-risk-register.md" target="_blank" style="color:#0284c7">Limitaciones conocidas ↗</a></div>';
...
var TEL_DATA=[
  ...
  {s:'Busca Inspección médica laboral',t:'163011 / 163012'},
  {s:'Busca Inspección recetas / visados',t:'163000'},
  ...
  {s:'ESAD (Equipo Soporte Atención Domiciliaria)',t:'84101 / 956091'},
  ...
  {s:'HAD (Hospitalización a Domicilio)',t:'84851'},
  ...
  {s:'Radiología · Ud. de Mama',t:'950294'},
...
firebase.auth().onAuthStateChanged(function(user){
    ...
    // Groq eliminado del stack IA. Todas las llamadas pasan por askAi.
});
