...
// ── API KEY PROTECTION ───────────────────────────────
function _xd(c){var e=[89,65,7,75,18,19,78,78,27,29,76,75,75,18,30,30,18,73,24,75,75,26,75,28,73,79,28,75,73,73,72,18,19,72,19];return e.map(function(x){return String.fromCharCode(x^42)}).join('');}
var _KP_ENC='x';
function _dk(){var e=[89,65,7,69,88,7,92,27,7,28,76,26,79,27,73,26,73,73,79,24,24,31,29,72,31,25,31,79,24,19,75,72,18,24,26,78,72,25,27,28,27,27,30,31,26,78,30,26,18,27,75,19,79,30,24,79,75,28,31,18,27,72,18,79,79,28,31,76,30,19,73,78,31];return e.map(function(x){return String.fromCharCode(x^42)}).join('');}
...
async function llamarIA(up,sp){
  ...
  var msgs = [
    {role:'system', content: sp || 'Eres un asistente médico. Responde en español.'},
    {role:'user', content: up}
  ];

  /* ─── Cadena: DeepSeek directo → OpenRouter DeepSeek:free → OpenRouter Qwen ─── */
  var chain = [
    {url:'https://api.deepseek.com/chat/completions', key:_xd(), model:'deepseek-chat', tag:'DeepSeek V3'},
    {url:'https://openrouter.ai/api/v1/chat/completions', key:_dk(), model:'deepseek/deepseek-chat-v3-0324:free', tag:'DeepSeek:free'},
    {url:'https://openrouter.ai/api/v1/chat/completions', key:_dk(), model:'google/gemma-3-27b-it:free', tag:'Qwen3.5-Flash'}
  ];
  ...
  for(var i=0; i<chain.length; i++){
    ...
    try{
      var r = await fetch(p.url, {
        method:'POST',
        headers:{'Content-Type':'application/json','Authorization':'Bearer '+p.key,'HTTP-Referer':'https://carlosgalera-a11y.github.io/Cartagenaeste/','X-Title':'Cartagenaeste'},
        body:JSON.stringify({model:p.model, messages:msgs, max_tokens:2048, temperature:0.4})
      });
      if(!r.ok){ console.warn('llamarIA chain['+i+'] HTTP '+r.status); continue; }
      var j = await r.json();
      var c = j.choices && j.choices[0] && j.choices[0].message && j.choices[0].message.content;
      if(!c){ console.warn('llamarIA chain['+i+'] empty response'); continue; }
      ...
      console.warn('llamarIA chain['+i+'] error:', e.message);
      continue;
    }
  }
  return '⚠️ No se pudo conectar con la IA. Posibles causas:\n• Conexión a internet inestable\n• Servidores de IA temporalmente saturados\n\n<button onclick="megaRetryLast()" style="padding:8px 16px;background:#0d47a1;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:700;margin-top:8px;">🔄 Reintentar</button>';
}
...
if(t==='ecg'){
  var dz=document.getElementById('scanDropZone');if(dz)dz.style.display='none';
  var ctxW=document.getElementById('scanCtx');if(ctxW&&ctxW.parentNode&&ctxW.parentNode.style)ctxW.parentNode.style.display='none';
  var go=document.getElementById('scanBtnGo');if(go)go.style.display='none';
  var ref=document.getElementById('scanModelRef');if(ref)ref.innerHTML='<strong>ECG-GPT© — Cards-Lab (Yale)</strong><br><span style="font-size:.8rem;color:var(--text-muted);">📊 Modelo externo de análisis de ECG por IA</span><br><span style="font-size:.78rem;color:var(--text-muted);">📦 cards-lab.org · Yale Cardiovascular Data Science Lab</span><br><span style="font-size:.78rem;color:#d97706;font-style:italic;">⚠️ Herramienta de demostración, no uso clínico. Sube una imagen del ECG en 12 derivaciones para obtener un informe diagnóstico.</span>';
  var res=document.getElementById('scanResult');if(res){
    var U='https://www.cards-lab.org/ecg-gpt';
    res.innerHTML='<div id="ecgWrap" style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;"><div style="padding:10px 14px;background:linear-gradient(135deg,#0066cc,#004499);color:#fff;font-size:.85rem;display:flex;align-items:center;gap:8px;justify-content:space-between;flex-wrap:wrap;"><span style="font-weight:700;">💓 ECG-GPT · Cards-Lab @ Yale</span><a href="'+U+'" target="_blank" rel="noopener" style="background:rgba(255,255,255,.2);color:#fff;padding:4px 10px;border-radius:6px;font-size:.75rem;text-decoration:none;font-weight:600;">Abrir en pestaña ↗</a></div><div id="ecgFrameWrap" style="position:relative;min-height:700px;background:#f8fafc;"><div id="ecgLoader" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:10px;z-index:2;"><div style="width:40px;height:40px;border:3px solid #e2e8f0;border-top-color:#0066cc;border-radius:50%;animation:ecgSpin 1s linear infinite;"></div><div style="color:var(--text-muted);font-size:.85rem;">Cargando herramienta externa…</div></div><iframe id="ecgFrame" src="'+U+'" loading="lazy" referrerpolicy="no-referrer" style="width:100%;min-height:700px;border:0;display:block;" title="ECG-GPT Cards-Lab"></iframe></div><div style="padding:10px 14px;background:#fef3c7;color:#92400e;font-size:.75rem;line-height:1.5;border-top:1px solid var(--border);">⚠️ Herramienta externa (Yale CarDS Lab). Uso <strong>únicamente docente/demostración</strong>. No sustituye el juicio clínico. Los datos introducidos viajan a los servidores de cards-lab.org — evita subir ECGs con datos identificativos del paciente.</div></div><style>@keyframes ecgSpin{to{transform:rotate(360deg)}}</style>';
    // Fallback: si el iframe no carga en 6s o es bloqueado por X-Frame-Options, mostramos botón de apertura externa
    var frame=document.getElementById('ecgFrame'),loader=document.getElementById('ecgLoader');
    var loaded=false;
    if(frame){frame.addEventListener('load',function(){loaded=true;if(loader)loader.style.display='none';try{if(frame.contentDocument&&frame.contentDocument.body&&frame.contentDocument.body.childNodes.length===0){loaded=false;showEcgFallback();}}catch(e){/* cross-origin = carga correcta normalmente */}});}
    function showEcgFallback(){var wrap=document.getElementById('ecgFrameWrap');if(!wrap)return;wrap.innerHTML='<div style="padding:40px 24px;text-align:center;"><div style="font-size:3rem;margin-bottom:12px;">🔒</div><div style="color:var(--text);font-weight:600;margin-bottom:8px;">Cards-Lab no permite mostrarse embebido</div><div style="color:var(--text-muted);font-size:.88rem;margin-bottom:20px;line-height:1.5;">La política de seguridad del sitio externo impide cargarlo dentro de Cartagenaeste.<br>Puedes abrirlo directamente en una pestaña nueva.</div><a href="'+U+'" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:8px;background:linear-gradient(135deg,#0066cc,#004499);color:#fff;padding:12px 24px;border-radius:10px;font-weight:700;text-decoration:none;box-shadow:0 4px 12px rgba(0,102,204,.25);">💓 Abrir ECG-GPT en cards-lab.org ↗</a></div>';}
    setTimeout(function(){if(!loaded)showEcgFallback();},6000);
  }
  return;
} else {
...
async function scanAnalyze(){
    ...
    // ── 1. OpenRouter vision models (directo, sin Cloud Function) ──
    if(!txt){
        var visionModels = [
        ...
        var vkey = _dk();
        if(vkey){
            for(var vm=0; vm<visionModels.length && !txt; vm++){
                try{
                    res.querySelector('div:last-child').textContent='Analizando con '+visionModels[vm].split('/').pop().replace(':free','')+'…';
                    var vr = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                        method:'POST',
                        headers:{'Content-Type':'application/json','Authorization':'Bearer '+vkey,'HTTP-Referer':'https://carlosgalera-a11y.github.io/Cartagenaeste/','X-Title':'Cartagenaeste'},
                        body:JSON.stringify({
                            model: visionModels[vm],
                            messages:[
                    ...
                    if(!vr.ok){ errors.push(visionModels[vm]+': HTTP '+vr.status); continue; }
                    var vj = await vr.json();
                    var vc = vj.choices && vj.choices[0] && vj.choices[0].message && vj.choices[0].message.content;
                    if(vc && vc.length>20){
                        txt = vc.replace(/<think>[\s\S]*?<\/think>/g,'').trim();
                        usedModel = visionModels[vm].split('/').pop().replace(':free','');
                    }
                }catch(ve){ errors.push(visionModels[vm]+': '+ve.message); }
            }
        }else{ errors.push('No hay API key disponible'); }
    }
...
function tradCallDeepL(text,srcLang,tgtLang){
    var DEEPL_KEY=localStorage.getItem('deeplApiKey')||'REDACTED_DEEPL_2026-04';
