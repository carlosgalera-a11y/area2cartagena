...
DS_KEYS=$(echo "$HTML" | grep -c "sk-6a5ea8dfa7d64c929dad02907917979f")
...
DS_MOD=$(echo "$MODULES" | grep -c "sk-6a5ea8dfa7d64c929dad02907917979f")
...
NAS_HTTP=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 3 "http://192.168.1.35:3100/health" 2>/dev/null)
if [ "$NAS_HTTP" = "200" ]; then
  green "NAS proxy accesible"
  # Test AI endpoint
  AI_HTTP=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 5 -X POST "http://192.168.1.35:3100/ai/chat" -H "Content-Type: application/json" -d '{"messages":[{"role":"user","content":"test"}],"max_tokens":5}' 2>/dev/null)
